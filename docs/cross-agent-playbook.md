...
5. **交互弹窗与自动化纪律（2026-09-10 实战案例；初版归因有误，经用户澄清修正）**：真实时间线——Codex 弹 A/B/C 裁决问题后结束回合，弹窗随轮次消亡（widget 绑定活动轮次）；用户发情况说明后 Codex **重弹选择**，用户选定 A+A1，裁决经 L1 正常转送 L2。Codex 处置全程模范：拒绝代答、主动暂停定时跟进、自愈式重弹。预防性纪律三条：①自动化/巡检轮遇「待用户裁决的挂起问题」不得代答——以纯文本重呈问题后结束本轮；②裁决类请求一律**文本+弹窗双轨**发出（弹窗随轮次易逝，文本跨轮存活——本次弹窗消失恰因缺文本轨）；③人机交互密集期（实战指挥中）heartbeat 降档 30 分钟或暂停
